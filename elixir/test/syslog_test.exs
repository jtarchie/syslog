defmodule SyslogTest do
  ...
  test "parses a valid message" do
    {status, _} = Syslog.parse(@valid_message)
    assert status == :ok
  end

  test "sets the version" do
    {:ok, log} = Syslog.parse(@valid_message)
  ...
  test "sets values to null when set to '-'" do
    {:ok, log} = Syslog.parse(~S(<34>1 2003-10-11T22:14:15.003Z - - - - -))
  ...
  test "example timestamps from the RFC can be parsed" do
    {:ok, log} = Syslog.parse(~S(<34>1 2003-10-11T22:14:15.00003Z - - - - -))
    assert DateTime.to_iso8601(log.timestamp) == "2003-10-11T22:14:15.00003Z"

    {:ok, log} = Syslog.parse(~S(<34>1 1985-04-12T23:20:50.52Z - - - - -))
    assert DateTime.to_iso8601(log.timestamp) == "1985-04-12T23:20:50.52Z"

    {:ok, log} = Syslog.parse(~S(<34>1 1985-04-12T23:20:50.52+00:00 - - - - -))
    assert DateTime.to_iso8601(log.timestamp) == "1985-04-12T23:20:50.52Z"
  ...
  test "date can be nil" do
    {:ok, log} = Syslog.parse(~S(<34>1 - - su - - - 'su root' failed for lonvick on /dev/pts/8))
    assert log.timestamp == nil
  end

  test "ignores unparseable timestamps" do
    {:ok, log} =
      Syslog.parse(~S(<34>1 asdfasdfasdf - su - - - 'su root' failed for lonvick on /dev/pts/8))

    assert log.timestamp == nil
  end

  test "reports errors" do
    {:error, msg} = Syslog.parse("")
