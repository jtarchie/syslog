...
const syslog_rfc5424_first_final int = 585
...
tr3:
//line parse.rl:62
 pe, eof = atoi(data[ts:p]) + (p-ts) + 1, atoi(data[ts:p]) + (p-ts) + 1 
	goto st3
	st3:
		if p++; p == pe {
			goto _test_eof3
...
tr15:
//line parse.rl:65
 log.hostname = toString(data[ts:p]) 
	goto st14
	st14:
		if p++; p == pe {
			goto _test_eof14
...
tr18:
//line parse.rl:66
 log.appname = toString(data[ts:p]) 
	goto st16
	st16:
		if p++; p == pe {
			goto _test_eof16
